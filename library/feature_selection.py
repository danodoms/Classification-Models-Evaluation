...
class Classification:
    ...
    def perform_anova(dataframe, dependent_var, independent_vars=None):
        results = []
        
        # Determine independent variables
        if independent_vars is None:
            independent_vars = [col for col in dataframe.columns if col != dependent_var]
        
        # Convert dependent variable to numeric if it's not already
        if dataframe[dependent_var].dtype == 'object':
            le = LabelEncoder()
            dataframe[dependent_var] = le.fit_transform(dataframe[dependent_var])
        
        for ind_var in independent_vars:
            if ind_var != dependent_var:  # Ensure independent and dependent variables are not the same
                groups = [dataframe[dataframe[ind_var] == group][dependent_var] for group in dataframe[ind_var].unique()]
                f_stat, p_value = f_oneway(*groups)
                formatted_p_value = float("{:.20f}".format(p_value))  # Convert to regular decimal form
                results.append({
